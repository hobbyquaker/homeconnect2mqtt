...
- Events are applied through `appliance.apply(item)` which also feeds derived items
  (`finish_at`, `alert/*`) and the `key/*` mirror.
...
OQ-H3), `--raw-set` (off), `--login` (action), `--dump` (action: write fixtures). `mqttInterfaces:
...
- The current repo content (`package.json` 0.0.1, empty `index.js`, README stub, LICENSE) is
  the **npm name reservation** — `npm publish --otp=<code>` was still pending on 2026-08-26
  because of 2FA. Replace with the real skeleton in milestone 0.2; keep `version` < 1.0 until
  the flow is retired.
