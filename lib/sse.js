...
export class EventStream extends EventEmitter {
     ...
     * @param {number} [options.flapWindow] ms a stream must stay open before the backoff resets (default 10 s)
    ...
    async connect() {
        ...
        try {
            ...
            this.emit('open');
            this.resetWatchdog();
            // flap damping: a stream that dies within `flapWindow` of opening does not reset the
            // backoff, otherwise a server that accepts and immediately drops us loops fast
            const damp = this.setTimer(() => {
            ...
            try {
                ...
                clearTimeout(damp);
                if (Date.now() - openedAt >= this.flapWindow) {
                    this.backoff = this.minBackoff;
                }
            }
            // clean end of stream
