...
# homeconnect2mqtt@<name> services there. Same script as in lgtv2mqtt / lgsb2mqtt.
#
# Usage:
...
#
# Optional env vars:
#   REMOTE_DIR   (default: /usr/local/lib/node_modules/homeconnect2mqtt)
#   REMOTE_TMP   (default: /tmp)
#   SERVICE      systemd unit(s) to restart after deploying; default: every active
#                homeconnect2mqtt@<name> instance plus a plain homeconnect2mqtt unit if present
...
# mqtt-interfaces-core) are packed and shipped too; registry versions need nothing extra.
#
# The deploy does not create a service; install one once on the target with
#   sudo homeconnect2mqtt --install -n homeconnect --client-id ID -u mqtt://broker

PKG=homeconnect2mqtt
...
if [[ -n "$SERVICE" ]]; then
  ...
  echo "No $PKG service found to restart (install one with: sudo $PKG --install -n homeconnect --client-id ID -u mqtt://broker)"
fi
